...
def run_luna_max(config: MemeBotConfig, snapshot: dict[str, Any]) -> MarketPolicy:
    ...
    prompt = (
        ...
        "shorts. Evaluate both directions independently. Directional funding, an ineligible "
        "symbol, or missing/uncertain macro evidence must not pause every entry: return "
        "ALLOW_EVALUATION "
        "and let the downstream per-symbol, per-side quantitative gates decide. Use "
        "PAUSE_NEW_ENTRIES only when the exchange snapshot itself is missing, stale or unreliable, "
        "or an acute market-wide disruption makes both directions unsafe. Use web search only for "
        "current evidence from the allowed domains included below; macro evidence is context, not "
        "a mandatory trading signal. Include at least one fresh source from an allowed domain, "
        "preferably Bitunix for exchange conditions; sources must not be empty. Never place or "
        "suggest an order. Return only the requested JSON. allowed_strategies may contain only "
        "these exact identifiers: adaptive_range, "
        "breakout_retest, momentum_pullback. Risk multiplier may only reduce risk. Policy lifetime "
        "must be at most six hours.\n"
