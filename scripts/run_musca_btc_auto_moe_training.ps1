...
$status = "data/reports/musca_btc_auto_moe.status.json"
$stdout = "data/logs/musca-btc-auto-moe.stdout.log"
$stderr = "data/logs/musca-btc-auto-moe.stderr.log"
New-Item -ItemType Directory -Force -Path "data/logs", "data/reports" | Out-Null

$arguments = @("-m", "adaptive_bot.musca_btc_auto_moe")
...
while (-not $worker.HasExited) {
    Clear-Host
    Write-Host "MUSCA BTC - SCOPERTA ESPERTI + GATING GPU"
